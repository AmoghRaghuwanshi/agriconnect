...
const MANDI_DATA = [
  { crop: 'Wheat (Lokwan)', state: 'Madhya Pradesh', market: 'Indore', price: 2450, prev: 2380, unit: 'qtl', emoji: '🌾' },
  { crop: 'Rice (Basmati)', state: 'Punjab', market: 'Amritsar', price: 3800, prev: 3750, unit: 'qtl', emoji: '🍚' },
  { crop: 'Onion', state: 'Maharashtra', market: 'Nashik', price: 1200, prev: 1350, unit: 'qtl', emoji: '🧅' },
  { crop: 'Tomato', state: 'Karnataka', market: 'Kolar', price: 800, prev: 650, unit: 'qtl', emoji: '🍅' },
  { crop: 'Potato', state: 'Uttar Pradesh', market: 'Agra', price: 950, prev: 1050, unit: 'qtl', emoji: '🥔' },
  { crop: 'Maize', state: 'Bihar', market: 'Patna', price: 1850, prev: 1800, unit: 'qtl', emoji: '🌽' },
  { crop: 'Mustard', state: 'Rajasthan', market: 'Alwar', price: 5200, prev: 5100, unit: 'qtl', emoji: '🟡' },
  { crop: 'Chili', state: 'Andhra Pradesh', market: 'Guntur', price: 7800, prev: 8200, unit: 'qtl', emoji: '🌶️' },
  { crop: 'Soybean', state: 'Maharashtra', market: 'Latur', price: 4650, prev: 4500, unit: 'qtl', emoji: '🫘' },
  { crop: 'Cotton', state: 'Gujarat', market: 'Rajkot', price: 6800, prev: 6900, unit: 'qtl', emoji: '🧶' },
  { crop: 'Sugarcane', state: 'Uttar Pradesh', market: 'Meerut', price: 385, prev: 375, unit: 'qtl', emoji: '🎋' },
  { crop: 'Turmeric', state: 'Telangana', market: 'Nizamabad', price: 9200, prev: 8800, unit: 'qtl', emoji: '🟡' },
];

const STATES = ['All States', 'MP', 'Maharashtra', 'UP', 'Punjab', 'Karnataka', 'Bihar', 'Gujarat', 'AP', 'Rajasthan'];
...
export default function MandiPage() {
  ...
  const filtered = MANDI_DATA.filter(row => {
    const matchSearch = !search || row.crop.toLowerCase().includes(search.toLowerCase());
    const matchState = stateFilter === 'All States' || row.state.toLowerCase().includes(stateFilter.toLowerCase());
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 'var(--radius-md)', padding: '1rem 1.5rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 800 }}>18</div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 'var(--radius-md)', padding: '1rem 1.5rem', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.7rem', opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Crops</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="container" style={{ padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: '1 1 280px' }}>
            ...
            <input className="input" placeholder="Search crop name..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          {STATES.map(s => (
            <button key={s} onClick={() => setStateFilter(s)}
              style={{
                ...
                fontSize: '0.8rem', fontWeight: stateFilter === s ? 600 : 400, cursor: 'pointer', whiteSpace: 'nowrap',
                background: stateFilter === s ? 'var(--green-900)' : 'var(--bg-card)',
                color: stateFilter === s ? '#fff' : 'var(--text-secondary)',
                boxShadow: stateFilter === s ? 'none' : '0 1px 3px rgba(0,0,0,0.06)',
        ...
        {/* Price cards (mobile) + Table (desktop) */}
              ...
              {filtered.map((row, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? '#fff' : '#F9F6F0', transition: 'background 0.15s' }}
                    onMouseEnter={(e: React.MouseEvent<HTMLTableRowElement>) => (e.currentTarget.style.background = '#F0F4F0')}
                    onMouseLeave={(e: React.MouseEvent<HTMLTableRowElement>) => (e.currentTarget.style.background = i % 2 === 0 ? '#fff' : '#F9F6F0')}>
